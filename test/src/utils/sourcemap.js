...
function loadSourceMap(data) {
  return new Promise((resolve) => {
    fetch(`http://localhost:8083/getmap`, {
      method: 'POST',
...
export const findCodeBySourceMap = async ({ fileName, line, column }, callback) => {
  let sourceData = await loadSourceMap({ fileName, line, column });
  ...
  for (let i = start; i <= end; i++) {
    j++;
    newLines.push(
      `<div class="code-line ${i + 1 == row ? 'heightlight' : ''}" title="${i + 1 == row ? result.source : ''}">${j}. ${repalceAll(
        codeList[i]
      )}</div>`
    );
  }
