import { ResourceTarget, ResouceError, HttpData } from '@webmon/types/src/core/base';
import { fromHttpStatus, getTimestamp, interceptStr } from '@webmon/utils/src/core/helpers';
...
export function resourceTransform(target: ResourceTarget): ResouceError {
  return {
    time: getTimestamp(),
    message: (interceptStr(target.src as string, 120) || interceptStr(target.href as string, 120)) + '; 资源加载失败',
